...
import os
import re
from prepare_data_with_resp_labcorr import *
...
def extract_ts(X_ts_train, X_ts_test, y_train, y_test, method='PCA', num_samples=5, summarization_int=1):
    ...
    if method == 'PCA':
        ...
        return X_ts_train, X_ts_test, tsfresh_feat
...
def stack_data(rld, reprocess, loaded_loc, processed_loc, data_dir, summarization_int):
    ...
    if num_features <= 4:
        ...
        X_stacked = np.hstack([X_ts, X_lab, X_resp_tb, X_med, X_inf, X_dem, X_nc, X_aperiodic])

    elif num_features <= 7:
        num_components.append(X_aperiodic.shape[1])
        X_stacked = np.hstack([X_ts, X_lab, X_resp_tb,  X_med, X_inf, X_dem, X_aperiodic])
...
def main():
    ...
    for run in tqdm(range(int(args.num_runs))):
        ...
        if len(num_components) > 5:
            ...
            print(X_inf_train.shape)
            print(X_nc_train.shape)
            print(X_aperiodic_train.shape)
